...
public class ByteUtils {
    ...
    public static byte[] uuidToByteArray(String uuidStr) throws Exception {
        if (uuidStr.indexOf('-') != -1) {
            uuidStr = uuidStr.replaceAll("-", "");
        }
        if (uuidStr.length() != 32 && isHexString(uuidStr, null)) {
            throw new Exception("Illegal UUID String");
        }
    ...
    public static String byteArrayToUUIDString(byte[] bytes) throws Exception {
        if (bytes.length != 32) {
            throw new Exception("Illegal UUID byte array");
        }
     ...
     *                  分隔符不能为16进制数
     * */
    public static byte[] hexStringToByteArray(String hexString, Character delimiter) throws Exception {
        if (!isHexString(hexString, delimiter)) {
            throw new Exception("Illegal HexString");
        }
        if (delimiter == null) {
